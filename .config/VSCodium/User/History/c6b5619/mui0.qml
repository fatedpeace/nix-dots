...
PanelWindow{ // the parent, outter most
    ...
    Rectangle { // the child insde the parent
        ...
        //y: islandVisible ? 8 : -60 // dectects the value for the property islandVisibile and if true it sets y = 8 and if false it sets it to y = -60, using 8 as indirect way to set margin bc if set via margins.top the entire panel would start 8px down which would make mouse area 8px from top of screen
