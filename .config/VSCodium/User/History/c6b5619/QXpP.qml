...
PanelWindow{ // the parent, outter most
    ...
    implicitHeight: island.height + 20 // the space reserved for the island to be automatically the height of the island the plus 8 is to adjust for the 8px margins
    ...
    Rectangle { // the child insde the parent
        ...
        y: -60 // dectects the value for the property islandVisibile and if true it sets y = 8 and if false it sets it to y = -60, using 8 as indirect way to set margin bc if set via margins.top the entire panel would start 8px down which would make mouse area 8px from top of screen
